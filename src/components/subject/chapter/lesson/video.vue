<template>
    <div>
        <el-form :model="lessonInput" v-loading="loading" label-width="90px">
            ...
            <el-form-item label="视频来源" prop="name">
                <el-radio-group v-model="source">
                    <el-radio :label="3">上传视频</el-radio>
                    <el-radio :label="6">从资料库中选择</el-radio>
                </el-radio-group>
            </el-form-item>

            <el-form-item label="视频" prop="scope" v-if="source === 3">
            ...
            <el-form-item label="视频" prop="scope" v-if="source === 6">
        ...
        methods: {
            ...
            fetchData() {
                resourceList(this.lessonInput.courseId).then(response => {
                    if (response.message === 'success') {
                        this.options = response.data.filter(x => this.isVideo(x.filename));
                    } else {
                        this.$message.error("操作失败");
                    }
                }).catch(error => {
